 ...
 * discarded. *)
val status      : 'a t -> int option
(** Retrieve the termination status of an exhausted shtream.  If the
 * shtream still has data, returns [None].  Otherwise, [Some 0]
 * indicates normal termination and [Some n] for non-zero [n]
 * indicates abnormal termination.  If a shtream was made from an
 * external process (via {!Shtream.of_command}, for example), then
 * [n] is the exit code of the process.  (If the process closes its
 * output and continues running, the shtream will terminate with
 * [Some 0] rather than wait for the process.)
 *)
 ...
 * code.  Calling [fail_with 0] is equivalent to raising {!Failure}.
 *)
