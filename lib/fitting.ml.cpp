...
module Make(Shtream : SHTREAM) = struct
  ...
  let rec run_low = {
            ...
            Shtream.from_low
              ...
              {| maybe !snd
                 {| maybe (Shtream.next' s1)
                    {| let status = maybe (Shtream.status s1) {| 0 |} id in
    ...
    s_c = (fun fitting s out_w -> match fitting with
      ...
      | Seq r      -> let proc, _ = open_thunk {|
                      ...
                      |} in
                      let status = match Proc.wait proc with
                                   | Unix.WEXITED n -> n
                                   | Unix.WSIGNALED n -> 1024 + n
                                   | _ -> 0 in
  ...
  let (^>>=)    = seq
  let (^>>) a b = a ^>>= {| b |}
  let (&&^) a b = a ^>>= function 0 -> b | n -> yield n
  let (||^) a b = a ^>>= function 0 -> yield 0 | _ -> b
  let (~>>) = function
  ...
  let commands = 
    (function
       | x::xs -> List.fold_left (-|) x xs
       | []    -> yield 0) % (List.map command)

  let caml f  = yield 0 ^>>= {| f () |}
