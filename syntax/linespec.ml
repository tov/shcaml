...
and simple = {
  ...
  flag     : flag;
  typ      : MLast.ctyp;
  exp      : MLast.expr option;
}
and letval = {
  lloc     : MLast.loc;
  ldoc     : string option;
  lname    : string;
  ltyp     : MLast.ctyp option;
  lexp     : MLast.expr;
}
and typedec = {
  tloc     : MLast.loc;
  tdoc     : string option list;
  tdec     : MLast.type_decl list;
  tpvt     : bool;
  thid     : bool;
}
 ...
 *)
module Gram = Grammar.GMake(struct
  type te = Token.t
  let lexer = Grammar.glexer Pcaml.gram
end)
 ...
 *)
let l_linespec     = Gram.Entry.create "l_linespec"
let l_linespec_eoi = Gram.Entry.create "l_linespec_eoi"

let c_expr = Gram.Entry.of_parser "c_expr"
               (Grammar.Entry.parse_token Pcaml.expr) in
let c_ctyp = Gram.Entry.of_parser "c_ctyp"
               (Grammar.Entry.parse_token Pcaml.ctyp) in
let c_type_declaration
           = Gram.Entry.of_parser "c_type_declaration"
               (Grammar.Entry.parse_token Pcaml.type_declaration) in
GEXTEND Gram
  ...
  l_default:
    ...
    [[ "let"; LIDENT "default"; ":"; t = c_ctyp; "="; e = c_expr
         -> { dloc = _loc; dexp = e; dtyp = t; }
    ]]
    ;

  l_field:
    [[ "val"; (f, x) = l_field_helper; t = c_ctyp;
       e = OPT [ "="; e = c_expr -> e ]; d = l_doc
         -> Simple {
              sloc     = _loc;
              sdoc     = d;
              name     = x;
  ...
  l_field_helper:
    [[ x = QUESTIONIDENT; ":"      -> (Option, x)
     | x = OPTLABEL                -> (Option, x)
     | x = TILDEIDENT; ":"         -> (Label, x)
     | x = LABEL                   -> (Label, x)
...
module CtypMap = Map.Make(struct
  ...
  let rec t = function
    ...
    | TyObj (_, x, y) -> let f (a, b) = (a, t b) in
                         TyObj (p, map f x, y)
    | TyOlb (_, x, y) -> TyOlb (p, x, t y)
    | TyPol (_, x, y) -> TyPol (p, x, t y)
    | TyQuo (_, x)    -> TyQuo (p, x)
    | TyRec (_, x)    -> let f (_, a, b, c) = (p, a, b, t c) in
                         TyRec (p, map f x)
    | TySum (_, x)    -> let f (_, a, b) = (p, a, map t b) in
                         TySum (p, map f x)
    | TyPrv (_, x)    -> TyPrv (p, t x)
    | TyTup (_, x)    -> TyTup (p, map t x)
    | TyUid (_, x)    -> TyUid (p, x)
    | TyVrn (_, x, y) -> TyVrn (p, map r x, y)
  and r = function
    | RfTag (x, y, z) -> RfTag (x, y, map t z)
    | RfInh x         -> RfInh (t x)
...
let print_location (pos, _) =
  Printf.printf "# %d \"lib/line.ls\"\n"
    pos.Lexing.pos_lnum (* String.escaped pos.Lexing.pos_fname *)
...
and print_doc_item : doc_item -> unit = function
  | DocOne (_loc, doc, item) ->
      print_doc_string doc;
      print_location _loc;
      printf "%s\n\n" (Pcaml.string_of Pcaml.pr_sig_item item)
  | DocType (_loc, docs, decls) ->
      ...
      List.iter2 (fun doc (((_loc, _), _, _, _) as decl) ->
        print_doc_string doc;
        print_location _loc;
        let str = Pcaml.string_of Pcaml.pr_sig_item
                    (SgTyp (_loc, [decl])) in
        if !first
        then begin
          first := false;
          printf "%s\n\n" str
               ...
               (String.sub str 4 (String.length str - 4))) docs decls
  | DocMany (_loc, items) ->
      print_location _loc;
      List.iter print_doc_item items
...
let defaults_of_linespec ls =
  ...
  fun t -> try CtypMap.find t map
    with Not_found ->
      prerr_string "Warning: Default expression for type `";
      prerr_string (Pcaml.string_of Pcaml.pr_ctyp t);
      prerr_string "' not found\n";
      prerr_string "Warning: Using Obj.magic instead\n";
      <:expr< Obj.magic 0 >>
...
let rec acc_of_field root swrap mwrap fields =
  ...
  | Simple {
      ...
      name     = n;
      typ      = t;
    } -> let sel_row = swrap <:ctyp< < $n$: present .. > >> in
         doc_many _loc [
           doc_one _loc
           (match d with
      ...
      exp      = e;
         ...
         doc_many _loc [
           ...
           (let def = match e with
              | Some exp ->
                  sprintf ", default = [%s]"
                    (Pcaml.string_of Pcaml.pr_expr exp)
              | _ -> "" in
            match d with
              | Some s ->
  ...
  | Type {
      ...
      tdoc  = docs;
      tdec  = d;
    } -> let rhs ps = "abs_" ^ String.concat "" (List.map fst ps) in
         let erase (name, params, _, _) =
                   (name, params, TyQuo (_loc, rhs params), []) in
...
let type_decls_of_linespec ls =
  ...
  let rec tdol ty fields acc =
      ...
      List.fold_left
        (fun (curr, rest) each ->
           ...
           | Simple {
               ...
               name  = n;
               typ   = t;
             } -> ((_loc, subst (n :: ty), false, t) :: curr, rest)
           | Complex {
               cloc  = _loc;
               cname = n;
               fields = f;
             } -> let ty' = n :: ty in
                  ((_loc, subst (n :: ty), false,
                   TyLid (_loc, subst ty'))
                     :: curr,
                   tdol ty' f rest)
            | Type {
    ...
    let fields = fields_of_decls ls.decls in
    tdol [] fields
         [(_loc, "empty"), [], empty_record_of_fields _loc fields , []]
...
let mod_decls_of_linespec ls =
  ...
  let default_expr_of_field f
                 = match f.exp with
                   ...
                   | Some e -> e in
  let subst      = tysubst_of_linespec ls in
  let patt _loc ty    = <:patt< $lid: subst ty $ >> in
    ...
    <:expr<
      {
        $list:
          maption
            (function
             | Simple f
                 -> Some (patt f.sloc (f.name :: path),
                          default_expr_of_field f)
             | Complex c
                 -> Some (patt c.cloc (c.cname :: path),
  ...
  let make_create_function _loc path fields wrap =
    ...
    let def_arg field acc =
      ...
      let pattern = <:patt< $lid: rename field$ >> in
         <:expr< fun $PaOlb (_loc, field.name,
            Some (pattern, Some d))$ -> $acc$ >> in
    let lab_arg field acc =
      let pattern = <:patt< $lid: rename field$>> in
      <:expr< fun $PaLab (_loc, field.name, Some pattern)$ -> $acc$ >> in
    let exp_arg field acc =
  ...
  let make_type_aliases _loc =
    let alias ((_loc, name), _, _, _) =
          <:str_item< declare
            type $"root_"^name$ = $lid: name$;
            type $name$             = $lid: "root_"^name$;
          end >> in
    fun type_decls ->
